...
- **`lib/supabase/articles.ts`** — DB layer. `getPublishedArticles` + `getArticleBySlug` are wrapped in `unstable_cache` (revalidate 3600, tags `published-articles` / `article-by-slug`). `approveArticle`/`deleteArticle` call `updateTag(tag)` (from `next/cache`) for immediate invalidation — **never use `revalidateTag(tag, { expire: 0 })`**, that sets a 0-TTL policy on Vercel Data Cache which breaks `unstable_cache` for dynamically rendered article pages (500 errors on articles approved after deployment). `approveArticle` also stamps `published_at`. `getDraftArticles` is uncached.
